...
| New features without existing code alignment | Adding new structs/functions but not updating existing tools/MCP endpoints that should use them. BP037: added 218 templates but get_templates still returns 20 hardcoded. BP031: added Integration to CompetitorProfile but setup_competitive_analysis doesn't use it. | Every build plan must include an "existing code impact" audit: what existing tools, MCP endpoints, skills, and docs need updating to integrate the new feature? If a struct gets a new field, every tool that returns that struct must be checked. |
